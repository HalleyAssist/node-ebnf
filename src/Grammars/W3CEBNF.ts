...
namespace BNF {
  ...
  function getSubItems(tmpRules, seq: IToken, parentName: string, optionIndex: number, caseInsensitive: boolean = false, globalFragmentCounter?: {value: number}) {
    let anterior = null;
    let bnfSeq = [];
  ...
  function createRule(tmpRules: IRule[], token: IToken, name: string, caseInsensitive: boolean = false) {
